...
### Out of Scope
- Any external LLM/service integration in Phase 1.
...
- Intent Parsing (Phase 1):
  - Lightweight rule-based parser maps patterns to operations:
    - Sort: `sort by <col>( asc| desc)?(, <col> ...)?` with optional `nulls (first|last)`.
    - Remove column(s): `remove (column|columns) <names>`.
    - Rename: `rename <old> to <new>`.
    - Deduplicate: `deduplicate( by <cols>)?`.
    - Filter: `keep/remove rows where <col> <op> <value>`.
    - Reorder: `move step <n> (up|down|above <m>|below <m>)`.
    - Edit: `edit step <n> …` (re-parsed as an operation config change).
    - Parse config: `set sheet <name>`; `rows <start>-<end>`; `columns <start>-<end>`; `has headers (true|false)`.
  - Ambiguity triggers a question back to the user.
- Tooling Gateway:
  - Local functions operating on `steps` state and `upload.parseConfig` with dry-run → summarize → confirm → apply.
  - Preview sampling via existing `executeUntilStep()` returning a trimmed `ParseResult`.
- Extensibility (Phase 2):
  - Introduce a Convex action that calls an LLM with tools exposed via function-calling. Requires prior approval and a new spec revision.
...
## Testing Plan
- Unit tests for the rule-based intent parser covering:
  - Sort (single/multi-column, directions, nulls positioning)
  - Remove/rename column parsing with multi-word names
  - Deduplicate with/without columns
  - Filter operators (=, !=, contains, >, <, >=, <=)
  - Reorder and edit step commands
  - Parse config commands
  - Ambiguity/clarification prompts
- Integration (manual) on `/pipeline/[pipelineId]`:
...
- Implement rule-based intent parser in `src/lib/assistant/intent.ts` with unit tests.
